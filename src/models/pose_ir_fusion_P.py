...
class FUSION(nn.Module):
    ...
    def __init__(self, use_pose, use_ir, pretrained, fusion_scheme, batch_size):
        ...
        if use_pose:
            ...
            self.pose_net = SGN(60, 'NTU', 224, 1, batch_size) #SGN(args.num_classes, args.dataset, args.seg, args)
    ...
    def forward(self, X):
        ...
        *prime_X_fusion* function. Each tensor is then passed to its corresponding module. The 2 feature vectors are
        ...
        if self.use_pose:
            out_pose = self.pose_net(X_skeleton)[:, :, 0, 0]  # shape (batch_size, 512)        ###-------------------------------< changed
        if self.use_ir:
