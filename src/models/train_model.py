"""  Amine Notes:
the nn.DataParalel is avoided since we don't have 3 gpus
###############################################################################################
"""
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_DataParallel', default=False)
    ...
    seed = int(arg.seed)
    use_DataParallel = arg.use_DataParallel
    ...
    # Create model
    if model_type == "FUSION":
        model = FUSION(use_pose, use_ir, pretrained, fusion_scheme, batch_size)       ##################### modified: batch_size added
    else:
    ...
    # else:
    #     print("Now, let's use: ", torch.cuda.device_count(), "GPUs!")
