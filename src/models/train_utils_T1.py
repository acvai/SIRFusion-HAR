...
def evaluate_set(model, model_type, data_loader, output_folder, set_name):
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        ########################### -------------------------------------- this block need to be checked and re-injected, to be used in final training:
...
def train_model(model,
    ...
    for e in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_generator):
            ...
            # print(f" | X: {len(X)}")
            # ## to run billow => don't send X to 'cuda' above
            # print(f" | X0: {(torch.as_tensor(np.array(X[0])).size())}")
            # print(f" | X1: {(torch.as_tensor(np.array(X[1])).size())}")
            #
            # print(f" | Y: {Y.size()}")
            # assert False, '--------------a---------------'
                                                                  ########################################### commented bloc:
