...
class FUSION(nn.Module):
    ...
    def __init__(self, use_pose, use_ir, pretrained, fusion_scheme, batch_size):
        ...
        if use_ir:
            #self.ir_net = nn.Sequential(*list(r2plus1d_18(pretrained=pretrained).children())[:-1]) ##-------------------------< piste
            self.ir_net = main(cnn_name='resnet_amine',model_depth=18,n_classes=60,in_channels=3,sample_size=112)
        ...
        ### added Upsampling:
        t_, v_ = 20, 25
        self.pool3d = torch.nn.AvgPool3d([3, 1, 1])
        self.upsampling = torch.nn.Upsample(size=(1, t_, v_), mode='nearest')
    ...
    def forward(self, X):
        ...
        *prime_X_fusion* function. Each tensor is then passed to its corresponding module. The 2 feature vectors are
        ...
        if self.use_pose:
            ...
            out_pose, x_f = self.pose_net(X_skeleton)  ## x_f=(N, 256, 20, 25)
            out_pose = out_pose[:, :, 0, 0]  # shape (batch_size, 512)
        if self.use_ir:
            out_ir, out_layer_3 = self.ir_net(X_ir)   #out_ir=[bs, 512], out_layer_3=[bs, 256, 3, 7, 7]
            # out_ir = out_ir[:, :, 0, 0, 0]  # shape (batch_size, 512)

            out_layer_3 = self.pool3d(out_layer_3)                      # [bs, c:256, 1, 8, 8]
            out_layer_3 = self.upsampling(out_layer_3)                  # [bs, c:256, depth:1, t:20, v:25]
            out_layer_3 = out_layer_3.permute(0, 1, 3, 4, 2).squeeze()  # [bs, c:256, t:20, v:25]

        ### added: intermidiate fusion:
        fuse = x_f + out_layer_3
