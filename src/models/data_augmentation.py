...
################# added function to get output as [3, 25, 20]

def stretched_image_from_skeleton_sequence_2(skeleton, c_min, c_max):
    max_frame = skeleton.shape[1]
    n_joints = skeleton.shape[2]

    ### Reshape skeleton coordinates into an image
    skeleton_image = np.zeros((3, max_frame, n_joints))
    skeleton_image = skeleton[:, :, :, 0] + skeleton[:, :, :, 1]
    # skeleton_image[:, :, 0:n_joints] = skeleton[:, :, :, 0]
    # skeleton_image[:, :, n_joints:2 * n_joints] = skeleton[:, :, :, 1]
    skeleton_image = np.transpose(skeleton_image, (0, 2, 1))

    ### Normalize (min-max)
    skeleton_image = np.floor(255 * (skeleton_image - c_min) / (c_max - c_min))
    # print('fct_2 1: ', skeleton_image.shape)

    ### Reshape image for ResNet
    # print('fct_2 2: ', skeleton_image.transpose(1, 2, 0).shape)
    skeleton_image = cv2.resize(skeleton_image.transpose(1, 2, 0), dsize=(20, 25)).transpose(2, 0, 1)

    return skeleton_image

def stretched_image_from_skeleton_sequence_3(skeleton, c_min, c_max):
    max_frame = skeleton.shape[1]
    n_joints = skeleton.shape[2]
...
def stretched_image_from_skeleton_sequence_4(skeleton, c_min, c_max):
    ### this fonction is used for STGCN integration to FUSION-model.
    ### we need N, C, T, V, M params. with this function we get what we need => C, T, V, M (N: from main function not this script)

    skeleton  = np.transpose(skeleton, (2, 1, 0, 3))  #[v, t, c, m]
    v, t, c, m = skeleton.shape
    skeleton = skeleton.reshape((v, t, c*m))
    skeleton_image = cv2.resize(skeleton, dsize=(20, 25)).transpose(2, 0, 1).reshape((-1, 20, v, m))
